...
function binariesIssuesForDisplay(res) {
    ...
    if (dockerRes && dockerRes.binariesVulns) {
        ...
        for (var _i = 0, _a = _.values(binariesVulns.affectedPkgs); _i < _a.length; _i++) {
            var pkgInfo = _a[_i];
            issues.push(chalk.bold.white("------------ Detected " + _.values(pkgInfo.issues).length + " vulnerabilities" +
                (" for " + pkgInfo.pkg.name + "@" + pkgInfo.pkg.version + " ------------"), '\n'));
            for (var _b = 0, _c = _.values(pkgInfo.issues); _b < _c.length; _b++) {
                var pkgIssue = _c[_b];
                var issueID = pkgIssue.issueId;
                var issueData = binariesVulns.issuesData[issueID];
                var issueHeading = createSeverityBasedIssueHeading(issueData.severity, issueData.type, issueData.packageName);
                issues.push(issueHeading +
